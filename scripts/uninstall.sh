...
rm -f \
  /etc/apt/sources.list.d/rime-ready.list \
  /usr/share/keyrings/rime-ready-archive-keyring.gpg
apt-get update
specs=()
for package in "${packages[@]}"; do specs+=("$package=$ubuntu_version"); done
apt-get install -y --allow-downgrades "${specs[@]}"
printf '已移除 rime-ready APT 源并恢复 Ubuntu 22.04 官方 Rime 软件包；用户词库和配置未删除。\n'
